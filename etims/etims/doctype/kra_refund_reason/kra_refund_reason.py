...
def sync_from_etims():
    ...
    country_codes = [d for d in kra_codes if d.get("cdCls") == "32"]

    if not country_codes:
        frappe.log_error("No refund reasons found with cdCls = 32", "ETIMS KRA Refund Reasons Sync Error")
        return
    ...
    if 'dtlList' not in country_code_data:
        frappe.log_error("No dtlList found in refund reasons data", "ETIMS KRA Refund Reasons Sync Error")
        return
    ...
    for country in country_code_data['dtlList']:
        if frappe.db.exists("KRA Refund Reason", {"code": country['cd']}):
            skip_count += 1
            continue
        ...
        try:
            ...
            frappe.log_error(
                f"Error syncing KRA Refund Reason {country.get('cd', 'Unknown')}: {str(e)}", 
                "ETIMS KRA Refund Reason Sync Error"
            )
            continue
...
class KRARefundReason(Document):
	pass
