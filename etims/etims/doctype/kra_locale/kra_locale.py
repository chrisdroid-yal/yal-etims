...
def sync_from_etims():
    ...
    country_codes = [d for d in kra_codes if d.get("cdCls") == "48"]

    if not country_codes:
        frappe.log_error("No locales found with cdCls = 48", "ETIMS KRA Locales Sync Error")
        return
    ...
    if 'dtlList' not in country_code_data:
        frappe.log_error("No dtlList found in locale data", "ETIMS KRA Locales Sync Error")
        return
    ...
    for country in country_code_data['dtlList']:
        if frappe.db.exists("KRA Locale", {"code": country['cd']}):
            skip_count += 1
            continue
        ...
        try:
            ...
            frappe.log_error(
                f"Error syncing KRA Locales {country.get('cd', 'Unknown')}: {str(e)}", 
                "ETIMS KRA Locales Sync Error"
            )
            continue
