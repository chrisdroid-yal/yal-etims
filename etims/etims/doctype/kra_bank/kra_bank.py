...
def sync_from_etims():
    ...
    country_codes = [d for d in kra_codes if d.get("cdCls") == "36"]

    if not country_codes:
        frappe.log_error("No banks found with cdCls = 36", "ETIMS KRA Banks Sync Error")
        return
    ...
    if 'dtlList' not in country_code_data:
        frappe.log_error("No dtlList found in bank data", "ETIMS KRA Banks Sync Error")
        return
    ...
    for country in country_code_data['dtlList']:
        if frappe.db.exists("KRA Bank", {"code": country['cd']}):
            skip_count += 1
            continue
        ...
        try:
            ...
            frappe.log_error(
                f"Error syncing KRA Banks {country.get('cd', 'Unknown')}: {str(e)}", 
                "ETIMS KRA Banks Sync Error"
            )
            continue
