...
def sync_from_etims():
    ...
    country_codes = [d for d in kra_codes if d.get("cdCls") == "33"]

    if not country_codes:
        frappe.log_error("No currencies found with cdCls = 33", "ETIMS KRA Currencies Sync Error")
        return
    ...
    if 'dtlList' not in country_code_data:
        frappe.log_error("No dtlList found in currency data", "ETIMS KRA Currencies Sync Error")
        return
    ...
    for country in country_code_data['dtlList']:
        if frappe.db.exists("KRA Currency", {"code": country['cd']}):
            skip_count += 1
            continue
        ...
        try:
            ...
            frappe.log_error(
                f"Error syncing KRA Currencies {country.get('cd', 'Unknown')}: {str(e)}", 
                "ETIMS KRA Currencies Sync Error"
            )
            continue
