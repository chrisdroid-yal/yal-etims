...
def sync_from_etims():
    ...
    country_codes = [d for d in kra_codes if d.get("cdCls") == "04"]

    if not country_codes:
        frappe.log_error("No tax codes found with cdCls = 04", "ETIMS KRA Tax Code Sync Error")
        return
    ...
    if 'dtlList' not in country_code_data:
        frappe.log_error("No dtlList found in tax code data", "ETIMS KRA Tax Code Sync Error")
        return
    ...
    for country in country_code_data['dtlList']:
        if frappe.db.exists("KRA Tax Codes", {"code": country['cd']}):
            skip_count += 1
            continue
        ...
        try:
            ...
            frappe.log_error(
                f"Error syncing KRA Tax Code {country.get('cd', 'Unknown')}: {str(e)}", 
                "ETIMS KRA Tax Code Sync Error"
            )
            continue
